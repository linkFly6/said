define(['jquery', 'avalon'], function ($, avalon) {
    var template = '<div class="CLASS_CONTAINER" ms-visible="visible" ms-class="uploading:uploading">\
    ...
    var widget = avalon.ui.upload = function (elem, data, vms) {
        var config = data.uploadOptions,
            ...
            fail = $.isFunction(config.fail) ? config.fail : function () { },
            ...
            vm.change = function (e) {
                ...
                if (config.filters !== '*') {
                    ...
                    if (!~ext || (!~filters.indexOf(ext) && type !== '' && !~filters.indexOf(type))) {
                        fail({ code: 2, msg: '上传的文件不是可以接受的文件类型' });
                        return;
                    }
                }
                if (config.size > 0 && size > config.size) {
                    fail({ code: 1, msg: '上传的文件超过了约定的最大大小（>' + Math.floor(config.size / 1024 / 1024) + 'MB）' });
                    return;
                }
                    ...
                    if (xhr.readyState === 4) {
                        if (xhr.status === 200) {
                            try {
                                ...
                                $.isFunction(config.done) && config.done(data);
                            } catch (e) {
                                $.isFunction(config.done) && config.done(xhr, { code: 3, msg: '服务器返回的并不是可解析的结果' });
                            }
                            //上传完成
                        } else {
                            fail({ code: 4, msg: '服务器返回异常' }, xhr);
                        }
                        vm.uploading = false;
