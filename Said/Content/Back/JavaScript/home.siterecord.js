define('home.siterecord', ['jquery', 'so', 'avalon', 'echarts', 'bsTable', 'bs-date'], function ($, so, avalon, ec) {
    ...
    return function () {
		...
		vmModel = avalon.define({
		    ...
		    startDate: null,
		    endDate: null,
		    containerAliyun: false,
		    toggleAliyun: function () {
		        vmModel.containerAliyun = !vmModel.containerAliyun;
		        vmModel.query();
		    },
		    query: function () {
		        //跳到第一页，直接重新加载数据
            ...
            var $bsTable = $('#btTable').bootstrapTable({
                queryParams: function (params) {
                    ...
                    // 0: 默认查询，过滤所有冗余数据， 1: 不过滤阿里云云盾的数据
                    params.filterType = vmModel.containerAliyun ? 1 : 0;
                    return params;
                },
                pageSize: 100,
