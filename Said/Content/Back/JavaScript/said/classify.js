define(['so', 'jquery', 'upload'], function (so, $) {
     ...
     * @param { string } config.defImg - 表示默认图，指定该项之后，如果没有上传图片，则使用默认图
        ...
        so.upload($fileInput[0], {
            ...
            callback: function (data, file) {
                if (data.error === 0) {
                    ...
                    if (config.callback)
                        config.callback.call($input, data, file);
                    $input.focus();
                } else
    ...
    UploadTest.prototype.val = function (value, imgValue) {
        ...
        if (imgValue != null) {//=>val(value,imgValue);
            ...
            if (this.config.defImg) {
                this.img = this.config.defImg;
                this.$img.attr('src', this.config.imgUrl + this.img).show(0);
                this.$fileInput.removeClass('hidden-upload');
            } else {
                this.$img.hide(0);
                this.img = '';
    ...
    */
    ...
    $.fn.toolTable = function () {
        ...
        $forms.eq(0).find('.btn-add').click(function () {
            //新增按钮
            $bsTable.trigger('add', [$addInput]);
        });
        $bsTable.on('changeForm', function (e, index) {
            toggleForm(index);
        });
        ...
        $editInput.on('keydown', function (e) {
            switch (e.keyCode) {
                case 13:
                    //不同的模式，触发不同的方法
                    $bsTable.trigger('save', [$editInput]);
                    break;
            }
        });
        $forms.eq(1).find('.btn-edit').click(function () {//编辑按钮
            $bsTable.trigger('save', [$editInput]);
        }).end().find('.btn-cancel').click(function () {//取消
            toggleForm(0);
        });
        return this;
    }
