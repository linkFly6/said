...
(function (factory) {
    ...
    */
    function highlightBlock(block) {
        ...
        //linkFly修改，原方法只操作DOM，新增的highlightInHTML需要返回结果
        return block;
    }

    /*
        linkFly改写 - 2016-08-21 02:08:03
        可直接传入HTML，然后返回一个JSON对象，为解析完成的格式
        由highlight自带的方法highlightBlock()生成，分析了一下，highlightBlock需要的参数block需要是这种格式的：
        {
            parentNode: { className: 'highlight highlight-source' },//用于判断highlight是否高亮这段代码
            className: 'javascript',//用于判断解析语言
            innerHTML: 'var foo="bar";',//解析的源码，如果配置了useBR为true，则会用到，用于解析\n为<br/>的
            textContent: 'var foo="bar";'//解析的源码，如果没有配置useBR为true，则会使用这个
        }

        这个方法可以直接在HTML中匹配到<pre><code></code><pre>标签并进行高亮操作
        **要求是传入的格式必须是highlight可接受的格式：<pre class="highlight"><code class="高亮语言">你的代码</code></pre>，并且不可嵌套<pre>标签**
    */
    var regHTMLTagPreAndCode = /<pre[\s\t]class=['"]([^'"]+)['"]><code[\s\t]class=['"]([^'"]+)['"]>([\S\s\t]*?)<\/code><\/pre>/gi;
    function highlightInHTML(html) {
        //匹配到所有<pre>标签，并进行处理
        return html.replace(regHTMLTagPreAndCode, function (sourceHTML, preClass, codeClass, code) {//[源码, <pre>标签的class, code标签的class, 要高亮的代码]
            var virtualDOM =
            {
    ...
    */
    ...
    //linkFly 因为Said里面用不到"页面加载的时候自动检测pre标签并高亮"这个功能，所以直接干掉这个功能
    /*
    ...
    hljs.highlightInHTML = highlightInHTML;//linkFly新增 - 2016-08-21 03:04:35 支持直接传入HTML高亮代码
    hljs.configure = configure;
    //hljs.initHighlighting = initHighlighting;
    //hljs.initHighlightingOnLoad = initHighlightingOnLoad;  linkFly  - 因为Said里面用不到"页面加载的时候自动检测pre标签并高亮"这个功能，所以直接干掉这个功能
